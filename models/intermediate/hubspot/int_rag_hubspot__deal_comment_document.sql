...
owners as (
    ...
    select *
    from {{ ref('unified_rag','stg_rag_hubspot__owner') }}
),
...
engagement_emails as (
    ...
    from {{ ref('unified_rag','stg_rag_hubspot__engagement_email') }} engagement_email
...
engagement_notes as ( 
    ...
    select 
        ...
        owners.owner_name,
        owners.owner_email
    from {{ ref('unified_rag','stg_rag_hubspot__engagement_note') }} engagement_note
    left join owners
...
email_comment_details as (
    ...
    select
        ...
        {{ fivetran_demo_downstream.coalesce_cast(["engagement_emails.commenter_email", "'UNKNOWN'"], dbt.type_string()) }} as commenter_email,
        {{ fivetran_demo_downstream.coalesce_cast(["engagement_emails.commenter_name", "'UNKNOWN'"], dbt.type_string()) }} as commenter_name,
        {{ fivetran_demo_downstream.coalesce_cast(["engagement_emails.title", "'UNKNOWN'"], dbt.type_string()) }} as email_title,
        {{ fivetran_demo_downstream.coalesce_cast(["engagement_emails.created_timestamp", "'1970-01-01 00:00:00'"], dbt.type_timestamp()) }} as comment_time,
        {{ fivetran_demo_downstream.coalesce_cast(["engagement_emails.body", "'UNKNOWN'"], dbt.type_string()) }}  as comment_body
...
note_comment_details as (
    ...
    select
        ...
        {{ fivetran_demo_downstream.coalesce_cast(["engagement_notes.owner_email", "'UNKNOWN'"], dbt.type_string()) }} as commenter_email,
        {{ fivetran_demo_downstream.coalesce_cast(["engagement_notes.owner_name", "'UNKNOWN'"], dbt.type_string()) }} as commenter_name,
        engagement_notes.title as engagement_note_title,
        {{ fivetran_demo_downstream.coalesce_cast(["engagement_notes.created_timestamp", "'1970-01-01 00:00:00'"], dbt.type_timestamp()) }} as comment_time,
        {{ fivetran_demo_downstream.coalesce_cast(["engagement_notes.body", "'UNKNOWN'"], dbt.type_string()) }} as comment_body
    from engagement_deals
    left join deals
        on deals.deal_id = engagement_deals.deal_id
        and deals.source_relation = engagement_deals.source_relation
